...
from knotvisualization import plot_video_2D, plot_video3D
import elastica as ea


class StretchingBeamSimulator(
    ea.BaseSystemCollection, ea.Constraints, ea.Forcing, ea.Damping, ea.CallBacks, ea.Contact,
):
    pass
...
stretch_sim.add_forcing_to(stretchable_rod).using(MultiTargetForce, force_mag,ramp_up_time=2,targets=targets)

#Snap forces  - certain nodes along the rod are pulled to intermediate targets to aid in forming the knot
snap_force_mag = 0.5
...
stretch_sim.detect_contact_between(stretchable_rod, stretchable_rod).using(
    ea.RodSelfContact, k=0.5 * 1e4, nu=3
)
...
class AxialStretchingCallBack(ea.CallBackBaseClass):
    """
    Tracks the velocity norms of the rod TODO redo this
    """
...
stretch_sim.collect_diagnostics(stretchable_rod).using(AxialStretchingCallBack, step_skip=20, callback_params=recorded_history)
